...
import com.redfinger.manager.modules.stat.service.StatFaultHandleService;

@Controller
@RequestMapping(value = "/stat/faultCeshiSolve")
public class FaultCeshiSolveController extends BaseController {


	@Autowired
	StatFaultHandleService service;
	...
	public Map<String,Object> getChart (HttpServletRequest request, HttpServletResponse response, Model model,StatDomain bean) {
		bean.setWhere(ConfigConstantsDb.configRoleCeshi());
		return service.statFualtSolveByRole(bean);
	}
	...
	public String export(HttpServletRequest request, HttpServletResponse response, Model model, String exportDatas, String exportHead, String exportField, String exportName) throws Exception {
		...
		for (int i = 0; i < lableList.size(); i++) {
			...
			domain.setId(numberList.get(numberList.size()/2+i));

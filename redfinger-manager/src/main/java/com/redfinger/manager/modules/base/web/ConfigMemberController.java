...
@Controller
@RequestMapping(value = "/base/configMember")
public class ConfigMemberController extends BaseController {
	...
	public void update(HttpServletRequest request, HttpServletResponse response, Model model, SysConfig config) throws Exception {
		Integer oldValue = Integer.parseInt(ConfigConstantsDb.configMaxBindPadCount());
		Integer newValue= Integer.parseInt(config.getMap().get("config_max_bind_pad_count").toString());
		int diff=newValue-oldValue;
		service.updateMap(request, config);
		service.updateConfigLeftBindPadCount(diff);
	}
}

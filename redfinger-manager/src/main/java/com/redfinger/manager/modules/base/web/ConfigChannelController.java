...
@Controller
@RequestMapping(value = "/base/configChannel")
public class ConfigChannelController extends BaseController{
	@Autowired
	...
	public void update(HttpServletRequest request, HttpServletResponse response, Model model, SysConfig config) throws Exception {
		Integer oldValue = Integer.parseInt(ConfigConstantsDb.configChannelMaxPadCount());
		Integer newValue= Integer.parseInt(config.getMap().get("config_channel_max_pad_count").toString());
		int diff=newValue-oldValue;
		service.updateMap(request, config);
		service.updateConfigChannelBindPadCount(diff);
	}
}

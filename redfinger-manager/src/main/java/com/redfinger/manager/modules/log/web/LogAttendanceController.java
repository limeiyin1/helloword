...
import com.redfinger.manager.modules.tasks.service.TaskAttendanceService;

@Controller
@RequestMapping(value = "/log/attendance")
public class LogAttendanceController extends BaseController {

	@Autowired
	TaskAttendanceService service;
	@Autowired
	...
	public PageInfo<TaskAttendance> list(HttpServletRequest request,
			HttpServletResponse response, Model model, TaskAttendance bean,
		...
		List<TaskAttendance> list = service.initQuery(bean)
				.andIn("userId", userId)
				.andGreaterThanOrEqualTo("attendanceTime", beginTime)
				.andLessThan("attendanceTime", endTime)
				.addOrderForce(bean.getSort(), bean.getOrder())
				.pageAll(bean.getPage(), bean.getRows());
		for (TaskAttendance task : list) {
		...
		PageInfo<TaskAttendance> pageInfo = new PageInfo<TaskAttendance>(list);
		return pageInfo;
	}
	...
	public String export(HttpServletRequest request,
			...
			TaskAttendance bean, String begin, String end,String userMobilePhone,String userEmail) throws Exception {
		...
		boolean keep = true;
		while (keep) {
			PageInfo<TaskAttendance> pageInfo = this.list(request, response, model, bean, begin, end, userMobilePhone,userEmail);
			List<TaskAttendance> list=pageInfo.getList();

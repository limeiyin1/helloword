...
@Controller
@RequestMapping(value = "/fault/pool")
public class FaultPoolController extends BaseController {
	...
	public void accept(HttpServletRequest request, HttpServletResponse response, Model model, RfFaultFeedback bean) throws Exception {
		service.accept(request, bean);
	}
	
	@RequestMapping(value = "batchAcceptForm")
	public String batchAcceptForm(HttpServletRequest request, HttpServletResponse response, Model model, RfFaultFeedback bean) throws Exception {
		return this.toPage(request, response, model);
	}
	
	@RequestMapping(value = "batchAccept")
	@ResponseBody
	public Object batchAccept(HttpServletRequest request, HttpServletResponse response, Model model, RfFaultFeedback bean,String padCodes) throws Exception {
		Map<String, String> errorMsg = Maps.newHashMap();
		String str = service.batchAccept(request, padCodes, errorMsg);
		return errorMsg;
		/*if(StringUtils.isNotBlank(str)){
			throw new BusinessException("设备编码为[" + str + "]的设备存在故障状态不能受理的状态！");
		}*/
	}
	...
	public void asyncExport(HttpServletRequest request, HttpServletResponse response, 
		...
		List<RfExport> rfExportts = exportService.initQuery()
				...
				.andEqualTo("type", ExportConstants.TYPE_FAULT_POOL)
		...
		export.setType(ExportConstants.TYPE_FAULT_POOL);

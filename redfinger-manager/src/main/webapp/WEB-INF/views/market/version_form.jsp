...
<body>
	...
	<script type="text/javascript">
		...
		function uploadFile(){
			...
			$("#module_submit_form").ajaxSubmit({
				...
				success: function (data) {
					...
					if(obj){
						if(obj.flag == 'true'){
							//document.getElementById('downloadUrl').innerHTML=obj.filePath;
							$('#downloadUrl').textbox("setValue", obj.filePath);
						}else{
							if(obj.msg){
								alert(obj.msg);
							}else{
		 ...
		 function uploadUpdateFile(){
			...
			$("#module_submit_form").ajaxSubmit({
				...
				success: function (data) {
					...
					if(obj){
						if(obj.flag == 'true'){
							 $('#updateUrl').textbox("setValue",obj.filePath);
						}else{
							if(obj.msg){
								alert(obj.msg);
							}else{
		...
		function copyUpdateFile(){
			...
			$('#downloadUrl').textbox("setValue", updateFile);
		}
	...
	<input type="hidden" name="versionId" value="${bean.versionId}">
	<table id="module_submit_table">
		<tr>
			<td class="td1">版本名称:</td>
			<td class="td2"><input class="easyui-textbox" type="text" name="versionName" value="${bean.versionName }" data-options="required:true,validType:'length[0,32]'"  /></td>
		</tr>
		<tr>
			<td class="td1">版本描述:</td>
			<td class="td2"><input style="height: 180px" class="easyui-textbox" type="text" name="versionDesc" value="${bean.versionDesc }"  data-options="required:true,validType:'length[0,500]',multiline:true" /></td>
		</tr>
		<tr>
			<td class="td1">版本编码:</td>
			<td class="td2"><input class="easyui-textbox" type="text" name="versionCode" value="${bean.versionCode }"  data-options="required:true,validType:'length[0,32]'" /></td>
		</tr>
		<tr>
...
<%--  		<tr>
			...
			<td class="td2"><input class="easyui-textbox" type="text" name="versionTime" value="${bean.versionTime }" data-options="required:true" /></td>
		</tr> --%>
		<tr>
			<td class="td1">更新地址:</td>
			<td class="td2">
			   <input class="easyui-textbox" type="text" id="updateUrl" name="updateUrl" value="${bean.updateUrl }"/>	
			</td>
		</tr>
		...
		<tr>
			...
			<td class="td2"><input class="easyui-textbox" type="text" name="versionNew" value="${bean.versionNew }" data-options="required:true,validType:'length[0,32]'" /></td>
		</tr>
		 -->
		<tr>
			<td class="td1">是否必须更新:</td>
			<td class="td2">
<!-- 				<input class="easyui-textbox" type="text" name="versionMust" value="${bean.versionMust }" data-options="required:true,validType:'length[0,32]'" /> -->
		...
		<tr>
			<td class="td1">备注:</td>
			<td class="td2"><input class="easyui-textbox" type="text" name="remark" value="${bean.remark }" data-options="required:true,validType:'length[0,32]'" /></td>
		</tr>
		<tr>
			<td class="td1">下载地址:</td>
			<td class="td2">
				<input  class="easyui-textbox" type="text" name="downloadUrl" id="downloadUrl" value="${bean.downloadUrl }"/><input class="disableClass" type="button" value="复制更新地址" onclick="copyUpdateFile();" id="copyBtn"/>
			</td>
		</tr>

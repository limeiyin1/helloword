...
function statSearch(){
	var legend = $("#where").combobox('getText');
	title =legend+"任务统计";
	var type=$("#type").combobox('getValue');
	var begin=$("#begin").datebox('getValue');
	var end=$("#end").datebox('getValue');
	var where=$("#where").combobox('getValue');
	if(type==""||begin==""|| end==""||where==""){
		$.messager.alert('操作失败', '查询条件不能为空！', "warning");
		return;
	}
	$.messager.progress();
...
<body>
	...
	<div id="module_container" >
		...
		<form id="module_search_form" class="easyui-form" method="post">
			...
			红豆任务：<select class="easyui-combobox input_width_default" editable="false" name="where" id ="where">
				<%-- <c:forEach items="${taskList}" var="bean">
				<option value="${bean.taskId}">${bean.taskName}</option>
				</c:forEach> --%>
			...
			<div class="module_search_button">
				...
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-search-rf" plain="false" onclick="statExport()">导出</a>
				<a href="javascript:void(0)" id="numAll" >&nbsp;&nbsp;&nbsp;总 计 :</a>
			</div>
		</form>
		</div>	

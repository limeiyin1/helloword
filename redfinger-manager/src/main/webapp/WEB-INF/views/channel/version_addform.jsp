...
<body>
	...
	<script type="text/javascript">
		...
		function uploadFile(){
			...
			$("#module_submit_form").ajaxSubmit({
				...
				success: function (data) {
					...
					if(obj){
						if(obj.flag == 'true'){
							$('#channelDownloadUrl').textbox({value:obj.filePath});
						}else{
							if(obj.msg){
								alert(obj.msg);
							}else{
		 ...
		 function uploadUpdateFile(){
			...
			$("#module_submit_form").ajaxSubmit({
				...
				success: function (data) {
					...
					if(obj){
						if(obj.flag == 'true'){
							 $('#channelUpdateUrl').textbox({value:obj.filePath});
						}else{
							if(obj.msg){
								alert(obj.msg);
							}else{
		...
		function copyUpdateFile(){
			...
			$('#channelDownloadUrl').textbox({value:updateFile});
		}
	...
	<form id="module_submit_form" class="easyui-form" method="post">
		...
		<tr>
			...
			<td class="td2"><input class="easyui-textbox" type="text" name="channelCode" data-options="required:true,validType:'length[0,32]'"  /></td>
		</tr>
		<tr>
			<td class="td1">版本编码:</td>
			<td class="td2"><input class="easyui-textbox" type="text" name="channelVersionCode" data-options="required:true,validType:'length[0,32]'"  /></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="td2">
			   <input class="easyui-textbox" type="text" id="channelUpdateUrl" name="channelUpdateUrl"/>	
			</td>
		</tr>
			<tr id="updateUrlTr">
				<td class="td1"></td>
		...
		<tr>
			...
			<td class="td2">
				<input  class="easyui-textbox" type="text" name="channelDownloadUrl" id="channelDownloadUrl"/><input class="disableClass" type="button" value="复制更新地址" onclick="copyUpdateFile();" id="copyBtn"/>
			</td>
		</tr>

...
var subusertaskview = $.extend({}, $.fn.datagrid.defaults.view, {
		...
		for(var i=0; i<fields.length; i++){
			...
			if (col){
				...
				if (col.checkbox){
					...
					if(rowData.subUserTaskCount && rowData.subUserTaskCount > 0){
						cc.push('<span class="datagrid-row-expander datagrid-row-expand" style="display:inline-block;width:16px;height:16px;cursor:pointer;" />');
					}else{
						cc.push('<div style="text-align:center;width:16px;height:16px;"></div>');
					}
		...
		return jq.each(function(){
			...
			function createGrid(target, conf, prow){
				...
				$(target)[plugin]($.extend({}, conf.options, {
					...
					view: (conf.subgrid ? deviceview : undefined),
